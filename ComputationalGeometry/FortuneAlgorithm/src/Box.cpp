...
bool Box::contains(const myVector2& point) const
{
    return point.x >= left - EPSILON && point.x <= right + EPSILON &&
        point.y >= bottom  - EPSILON && point.y <= top + EPSILON;
}

Box::Intersection Box::getFirstIntersection(const myVector2& origin, const myVector2& direction) const
{
...
int Box::getIntersections(const myVector2& origin, const myVector2& destination, std::array<Intersection, 2>& intersections) const
{
    // WARNING: If the intersection is a corner, both intersections are equals
    myVector2 direction = destination - origin;
