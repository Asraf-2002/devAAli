...
VoronoiDiagram::VoronoiDiagram(const std::vector<myVector2>& points)
{
...
bool VoronoiDiagram::intersect(Box box)
{
    ...
    for (const Site& site : mSites)
    {
        ...
        //--------> It's a dummy initialization of Box::side <-------
        Box::Side incomingSide = Box::Side::LEFT, outgoingSide = Box::Side::RIGHT;
        do
        {
...
VoronoiDiagram::Vertex* VoronoiDiagram::createVertex(myVector2 point)
{
...
VoronoiDiagram::Vertex* VoronoiDiagram::createCorner(Box box, Box::Side side)
{
    ...
    {
        case Box::Side::LEFT:
            return createVertex(myVector2(box.left, box.top));
        case Box::Side::BOTTOM:
            return createVertex(myVector2(box.left, box.bottom));
        case Box::Side::RIGHT:
            return createVertex(myVector2(box.right, box.bottom));
        case Box::Side::TOP:
            return createVertex(myVector2(box.right, box.top));
        default:
            return nullptr;
    }
}
