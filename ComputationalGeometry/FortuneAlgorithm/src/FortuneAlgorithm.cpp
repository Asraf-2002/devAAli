...
FortuneAlgorithm::FortuneAlgorithm(std::vector<myVector2> points) : mDiagram(std::move(points)){}
...
{
    double y;
    myVector2 convergencePoint = computeConvergencePoint(left->site->point, middle->site->point, right->site->point, y);
...
myVector2 FortuneAlgorithm::computeConvergencePoint(const myVector2& point1, const myVector2& point2, const myVector2& point3, double& y) const
{
    myVector2 v1 = (point1 - point2).getOrthogonal();
    myVector2 v2 = (point2 - point3).getOrthogonal();
    myVector2 delta = 0.5 * (point3 - point1);
    double t = delta.getDet(v2) / v1.getDet(v2);
    myVector2 center = 0.5 * (point1 + point2) + t * v1;
...
bool FortuneAlgorithm::bound(Box box)
{
    ...
    if (!mBeachline.isEmpty())
    {
        ...
        {
            // Bound the edge
            myVector2 direction = (leftArc->site->point - rightArc->site->point).getOrthogonal();
            myVector2 origin = (leftArc->site->point + rightArc->site->point) * 0.5f;
